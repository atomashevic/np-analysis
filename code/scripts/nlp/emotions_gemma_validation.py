    ...
    for i in tqdm(range(len(data))):
        ...
        if len(inputs) == BATCH_SIZE:
            ...
            for ie in range(len(results)):
                emotion = results[ie]
                if emotion in emotions_list:
                    res = [{'id':str(tids[ie]), 'emotion': str(emotion)}]
                    save_file(res, out_file)
                else:
                    print('Error', emotion)
                    res = [{'id':str(tid), 'emotion': 'nepoznato'}]
...
if __name__ == "__main__":
    ...
    for TEMP in [1.0, 0.0]:
        for MODEL_NAME in ["gemma4:31b",]:  #"gemma3:12b"]:
            ...
            #import data
            out_file = '../../results/emotions_gemma/emotions_%s_temp%s_validation.jsonl'%(MODEL_NAME, TEMP)
